<template>
  <RList :source="source">
    <template slot="item" slot-scope="resource">
      <router-link
        to="/resource/details"
        :title="resource.item.title"
        ><img
          class="cover"
          :src="resource.item.coverPath"
          style="height: 100px"
      /></router-link>
      <div class="details">
       <router-link
          to="/resource/details"
          :title="resource.item.title"
          ><span class="restitle"
            >{{resource.item.title}}</span
          ></router-link
        ><br />
        <p class="snippet">
			{{ resource.item.description }}
        </p>
        <div class="info">
          作者：<span>{{resource.item.author}}</span> 单位：<span>{{resource.item.authorCompany}}</span><br />
          上传日期：<span>{{resource.item.addTime}}</span> 点击量：<span>{{resource.item.viewCount}}</span>
          <el-tag type="danger"> 格式： {{resource.item.fileExtName}}</el-tag>
        </div>
      </div>
    </template>
  </RList>
</template>

<script>
import RList from "@/components/List";
export default {
  components: { RList },
  props: {
    source: { type: Array, dufault: [] },
  },
};
</script>

<style scoped>
</style>